...
TraceDrop(std::string dropTrFileName)
{
    ...
    else
    {
        ...
        Config::ConnectWithoutContext(
            "/NodeList/0/$ns3::Node/$ns3::TrafficControlLayer/RootQueueDiscList/3/Drop",
...
main(int argc, char* argv[])
{
    ...
    startTimeVar->SetAttribute("Max", DoubleValue(100.0)); // Adjust as needed

    Ptr<UniformRandomVariable> dataAmountVar = CreateObject<UniformRandomVariable>();
    dataAmountVar->SetAttribute("Min", DoubleValue(50000));  // Minimum bytes to send
    dataAmountVar->SetAttribute("Max", DoubleValue(500000)); // Maximum bytes to send

    // Bursty flows from sender node 1
    // Remove previous BulkSendApplication setup for bursty flows

// Bursty flows from sender node 1
for (uint32_t i = 0; i < 100; ++i)
{
    uint16_t port = 60000 + i; // Unique port for each flow
    ...
    // Generate random values for this flow
    double startTimeValue = startTimeVar->GetValue();

    // Configure OnOffApplication
    ...
    // Set On and Off times to create bursts
    // For example, On for 0.5s, Off for 1s, repeat
    onOffHelper.SetAttribute("OnTime", StringValue("ns3::ExponentialRandomVariable[Mean=0.5]"));
    onOffHelper.SetAttribute("OffTime", StringValue("ns3::UniformRandomVariable[Min=0.5|Max=10]"));

    // Install the application on sender node 1
    ApplicationContainer app = onOffHelper.Install(sources.Get(1));
    app.Start(Seconds(startTimeValue));
    app.Stop(Seconds(stopTime - 1));
}
